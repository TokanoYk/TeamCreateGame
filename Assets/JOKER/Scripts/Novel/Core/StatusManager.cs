...
namespace Novel{
	...
	public class ExpObject{
		...
		public static string replaceVariable(string str_right){
			...
			for (var i = 0; i < str_right.Length; i++) {
				...
				if (flag_var_now == true) {

					var_name += c;

				}

				if (c == "{" && flag_var_now == false) {
					flag_var_now = true;
	...
	public class Variable{
		...
		public void set(string key,string val){
			...
			if (type == "global") {
				...
				NovelSingleton.GameManager.globalSetting.globalVar[type] = variable_name;
	...
	public class StatusManager {
		...
		public static bool enableEventClick = true; //イベントをクリック検知できるかどうか event_stop でロックをかけることができる。 
